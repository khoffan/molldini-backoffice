<template>
  <div class="max-w-6xl mx-auto space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm sticky top-0 z-20 backdrop-blur-md bg-white/90">
    ...
    <div class="grid grid-cols-12 gap-6">
      <div class="col-span-12 lg:col-span-8 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-sm font-black text-slate-700 uppercase tracking-tighter">Service Details</h2>
          </div>
          <div class="p-6 space-y-5">
            <div class="grid grid-cols-2 gap-5">
              <div class="col-span-2 md:col-span-1">
                <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Shipping Name</label>
                <input 
                  v-model="form.name" 
                  type="text" 
              ...
              <div class="col-span-2 md:col-span-1">
                <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Estimated Days</label>
                <input 
            ...
            <div>
              <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Description</label>
              <textarea 
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-100 bg-emerald-50/30">
            <h2 class="text-sm font-black text-emerald-700 uppercase tracking-tighter">Pricing & Thresholds</h2>
          </div>
          <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Shipping Fee (฿)</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs">฿</span>
                <input v-model.number="form.price" type="number" class="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono font-bold outline-none focus:border-emerald-500 transition-all" />
              </div>
            </div>
            <div>
              <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Min Order (฿)</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs">฿</span>
                <input v-model.number="form.minOrderAmount" type="number" class="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono font-bold outline-none focus:border-emerald-500 transition-all" />
              </div>
            </div>
            <div>
              <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Free Delivery At (฿)</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs">฿</span>
                <input v-model.number="form.freeShippingThreshold" type="number" placeholder="No free" class="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono font-bold outline-none focus:border-emerald-500 transition-all" />
              </div>
            </div>
            <p class="col-span-full text-[10px] text-slate-400 font-medium italic">* หากยอดสั่งซื้อถึงเกณฑ์ที่กำหนด ระบบจะปรับค่าจัดส่งเป็น 0 บาทโดยอัตโนมัติ</p>
          </div>
        </div>
      </div>
      ...
      <div class="col-span-12 lg:col-span-4 space-y-6">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-sm font-black text-slate-700 uppercase tracking-tighter">Logistics Provider</h2>
          </div>
          <div class="p-6">
            <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Provider Type</label>
            <select 
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-sm font-black text-slate-700 uppercase tracking-tighter">Provider Logo</h2>
          </div>
          <div class="p-6">
            <div 
            ...
            >
              <img v-if="form.image" :src="form.image" class="w-full h-full object-contain p-4 transition-transform group-hover:scale-105" />
            ...
            <label class="block text-[11px] font-black text-slate-500 uppercase mb-2 ml-1">Direct Image URL</label>
            <input 
              v-model="form.image"
              type="text" 
